...
XmlElement.prototype._text = function(text) {
  ...
  if(this.options.textasnode){
    // create node for text value
    text = text.trim();
    if(text.length){
      var child = new XmlElement({ name: this.options.textnodename || 'text' });
      child.val = text;
...
function XmlDocument(xml, options) {
  ...
  this.parser = sax.parser(true, options); // strict
